...
object PaceHandWaveDetector {
  ...
  /** 격자 축 방향 게이트 — 2026-09-06 실측(39차단/2통과)으로 끔. 위 발화 지점 주석. */
  // 🔴 2026-09-06 다시 켬 — 끈 판단이 틀렸다.
  //   "39건 차단 / 2건 통과라 순 손해"라고 봤는데, **그 39건은 대부분 되돌아오는 손**이었을
  //   가능성이 크다(= 정상 동작). 그때 왼→오가 안 됐던 진짜 이유는 traverse 부호가 반대로
  //   박혀 있어 그 축이 통째로 막혀 있었던 것이고, 그건 별개로 고쳤다.
  //   차단 건수만 보고 "순 손해"라고 결론 낸 것이 오류다 — 차단이 많다는 것과 잘못 차단한다는
  //   것은 다르다. 라벨 측정(왼→오 = gross dir=+1, 4/4)은 그대로 유효하다.
  //   사장님 확인: traverse 정정 후 "많이 나아졌는데 오→왼이 한 번 됐다" — 그 누출 경로가
  //   방향 게이트가 없는 이 축이다.
  // 🔴 2026-09-06 최종 — 방향 게이트를 끈다. 라벨 측정 결과가 결론이다:
  //     왼→오 5번 → 2번 성공(40%)   오→왼 5번 → 3번 성공(60%)
  //   **막으려던 쪽이 오히려 더 잘 통과한다.** 방향 판정이 신호로서 신뢰할 수 없다는 뜻이고,
  //   로그에도 같은 왼→오에서 dir 이 +1/0/-1 로 다 나왔다(5건 중 2건은 dir=0, 즉 부호 없음).
  //   부호를 세 번 정정했지만 문제는 부호가 아니라 **판정 자체의 분산**이었다.
  //   되돌아오는 손 차단은 방향이 아니라 불응시간(REFRACTORY_MS)이 담당한다.
  private const val GROSS_DIRECTION_GATE = false
  // 🔴 2026-09-06 부호 정정 — 사장님 실기기("왼오 안 되고 오왼이 되는 게 두 번"):
  //   통과한 traverse 2건이 전부 dir=1 이었고 그때 하신 동작은 **오→왼**이었다.
  //   즉 traverse 에서 dir=+1 = 오→왼, 따라서 왼→오 = **-1**. 518행 주석이 맞았다
  //   ("사용자 왼→오른쪽이 이미지에서는 x 감소" → netX < 0).
  //   ⚠️ 내 오류의 정체: gross-motion 에서 잰 부호(+1)를 **계산 방식이 다른** traverse 에
  //     그대로 갖다 붙였다. 두 축은 방향을 다르게 만든다 —
  //       traverse      = 추적된 손의 netX 변위
  //       gross-motion  = 변화 영역 중심의 이동
  //     부호가 반대로 나오는 게 당연하다. 하나의 상수로 묶은 것 자체가 잘못이었다.
  private const val TRAVERSE_DIRECTION = 0 // 위 GROSS_DIRECTION_GATE 주석과 같은 이유 — 방향으로 거르지 않는다
  ...
  // 🔴 2026-09-06 5s → 10s. 실측 차단 집계에서 "손 미관측"이 19회로 압도적 1위였다(near-miss 13,
  //   역방향 11, 가로이동 6). 손짓하는 동안 팜 디텍터가 손을 놓치는 것이 최대 병목이다.
  //   오탐 위험은 "사람이 앞에 있는 동안"으로 제한되므로 창을 늘리는 대가는 작다.
  private const val LUMAPASS_HAND_RECENT_MS = 10000L
   ...
   */
  // 🔴 2026-09-06 실측 — 2.0 이 손짓 분포 **안쪽**에 들어와 작은 손짓을 자르고 있었다.
  //   사장님 "5번 중 마지막만 됨" 구간의 차단 로그가 전부 dx=1.5 였다(0.5칸 차이로 막힘).
  //   16칸 격자에서 1.5칸 = 화면 폭의 9% 다.
  //   앞서 잰 두 분포:  손 들기(오탐) dx=0.5~1.0 / 손짓(정탐) dx=1.5~5.0
  //   → 그 사이인 1.2 로 내린다. 2.0 은 정탐 구간을 침범한 값이었다.
  private const val GROSS_MIN_DX_CELLS = 1.2
  ...
  private fun checkGrossMotion(grid: IntArray, now: Long, onWave: () -> Unit) {
    ...
    if (ok && GROSS_MOTION_STANDALONE) {
      ...
      // 🔴 2026-09-06 껌 — 실기기에서 **39건 차단 / 2건 통과**로 순 손해였다.
      //   이 축의 "방향"은 200ms 간격 두 프레임의 **변화 영역 중심** 이동이라 손의 실제 이동
      //   방향을 제대로 따라가지 못한다(같은 왼→오 손짓도 -1 로 잡히는 경우가 많다).
      //   라벨 측정 4건이 전부 +1 이었던 것은 표본이 너무 적었기 때문이다.
      //   방향 판정은 추적된 손의 실제 변위를 쓰는 traverse 축에만 남긴다(TRAVERSE_DIRECTION).
      if (GROSS_DIRECTION_GATE && grossDir != 0 && grossDir != WAVE_DIRECTION) {
