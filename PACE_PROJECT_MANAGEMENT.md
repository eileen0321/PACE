...
**원인 두 겹**
1. music/gaming은 **원래 후보가 귀하다** — 인기 차트 상위 50개에 60초 이하가 거의 없다.
   이 파일 위쪽 `MAX_PAGES` 주석에 "music 0건, gaming 1건" 실측이 이미 남아 있었다.
2. 그 상태에서 YouTube가 429(쿼터 소진, 내가 refresh를 반복 호출해 태웠다)를 주자 채울 방법이 없었고,
   당시 코드가 **0건이어도 지우고 저장**해서 탭이 통째로 비었다.
