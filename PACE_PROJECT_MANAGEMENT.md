...
**Windows 세션(다음 작업)** → B1 완료, B2는 non-issue로 확인, B3(BOOT_COMPLETED)는 "재부팅 시 활성
세션 복구" 범위로 완료(실기기 미검증, 아래 §6 참고). B3 조사 중 발견한 "상시 사용량 감시 없음"
문제는 사장님 결정 대기(2-A로 승격할지, 그냥 알려진 설계로 둘지) — 다음 세션은 Home/온보딩/스플래시
WIP 스모크 테스트 및 커밋을 우선 진행 권장.
...
**사장님 결정 대기 중** → 2-A(D1~D6). 아무거나 하나씩 정리해주면 해당 세션이 나머지 코드
작업은 알아서 진행함. 순서 추천: D3(entitlement 여부, 시간 걸림 → 먼저 결정) → D1/D2(키·배포)
→ D4/D5(가벼움) → D6(제품 방향, 급하지 않음).
