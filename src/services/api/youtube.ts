...
export async function fetchShortsPage(opts: { query?: string; pageToken?: string | null } = {}): Promise<ShortsPage> {
  ...
  if (hasYouTubeProxy()) {
    ...
    //   2026-08-28 에 실제로 프록시가 0개를 주는 동안 앱이 통째로 멈췄다. 원인은 Vercel 빌드가
    //   11일간 실패해 낡은 코드가 서빙되고 있었던 것이다(@types/node 누락 → Buffer 타입 에러).
    //   ⚠️ 당시 나는 "유튜브가 데이터센터 IP 를 막는다"고 판단했으나 **틀렸다**. 그 근거였던
    //     비교 측정은 내 셸이 한글 검색어를 CP949 로 보내 서버가 깨진 검색어를 찾은 것이었다.
    //     제대로 인코딩해 다시 재니 서버 25/37/39/34/37 개, 가정용 IP 25/35/39/40/39 개로
    //     사실상 동일했고, 기본 경로도 12회 중 12회 성공(평균 82개)이었다. 유튜브는 우리를
    //     차별하지 않는다.
    //   그래도 이 폴백은 남긴다. 막는 주체가 유튜브가 아니라 **우리 서버가 죽는 것**이어도
    //   앱이 멈추는 건 똑같고, 실제로 11일짜리 배포 사고가 그렇게 났기 때문이다.
