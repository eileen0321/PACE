...
// "내려놓은 시간(쉬는 시간)"을 측정할 수 있게 onFlip 이벤트를 쏜다. (Android는 PaceFlipModule.kt가 동일 계약)
//
...
// 유지"돼야만 상태 전이(엎어놓기 2s / 집어들기 1s)한다. 0.5~0.8 사이는 히스테리시스 데드존.
//
// ⚠️ 앱이 백그라운드로 가면 CoreMotion 업데이트가 끊긴다(iOS 플랫폼 제약) — JS 쪽 useFlipStore가
//    AppState와 조합해 "포그라운드에서 엎어놓은 순간 ~ 다시 켜서 집어든 순간"으로 실경과를 보정한다.
//
// 🔒 스레드 안전(감사 C1/H1 반영): 모션 콜백(백그라운드 OperationQueue), isFaceDown/physicalFaceDown/stop
//    (동기 Function=JS 스레드), start(AsyncFunction=백그라운드)이 같은 상태(faceDown/candidateSince/
//    lastZ/hasSample)를 만진다 → 모든 접근을 lock으로 직렬화. 콜백 큐도 직렬(1)로. sendEvent는 형제
//    모듈(pace-volumekey/gesture) 관례대로 메인 스레드에서 발신하고, lock 밖에서 스냅샷 값으로 쏜다.
public class PaceFlipModule: Module {
  private let motion = CMMotionManager()
  private let queue: OperationQueue = {
    let q = OperationQueue()
    q.maxConcurrentOperationCount = 1 // 직렬 — 콜백 동시 실행/역순 처리 방지(H1)
    q.name = "com.pace.flip.motion"
    return q
  }()
  ...
  private var hasSample = false                    // 첫 샘플 도착 여부(stop/미시작 시 stale 조회 방지)
  ...
  public func definition() -> ModuleDefinition {
    ...
    // 디바운스 확정된 상태(관찰 중일 때만 유효). JS 스레드에서 호출 → lock.
    ...
    // background 복귀 후 재조율(그동안 집어들었는지)에 사용. 아직/더는 신선한 샘플 없으면 nil.
    ...
    AsyncFunction("start") { (promise: Promise) in
      ...
      // 이미 관찰 중이면 중복 start 방지 + 남은 큐 작업 취소(핸들러 이중 등록/구샘플 유입 차단).
      ...
      self.motion.startDeviceMotionUpdates(to: self.queue) { [weak self] data, error in
        guard let self = self, let z = data?.gravity.z else { return }
        var emit: Bool? = nil // non-nil이면 그 값으로 onFlip 발신
        self.lock.lock()
        self.lastZ = z
        self.hasSample = true
        ...
        if oppositeNow {
          if self.candidateSince < 0 {
            ...
            self.candidateSince = -1
            emit = self.faceDown // lock 안에서 스냅샷만 뜬다(밖에서 self.faceDown 재읽기 금지 — 레이스)
          }
        } else {
          self.candidateSince = -1 // 후보 상태가 흔들리면 리셋
        }
        self.lock.unlock()
        if let faceDownValue = emit {
          // 형제 모듈 관례대로 이벤트는 메인 스레드에서 발신(H3).
          DispatchQueue.main.async { self.sendEvent("onFlip", ["faceDown": faceDownValue]) }
        }
      }
      promise.resolve(nil)
    }
    ...
    Function("stop") {
      ...
      self.faceDown = false
      self.candidateSince = -1
      self.hasSample = false // 관찰 중단 후 physicalFaceDown이 stale 값을 반환하지 않게(H2)
      self.lock.unlock()
    }

    // 모듈 파괴(앱 컨텍스트 리로드/Fast Refresh 등) 시 센서 확실히 정지(M1 — 배터리/리소스 누수 방지).
    OnDestroy {
      self.motion.stopDeviceMotionUpdates()
      self.queue.cancelAllOperations()
    }
