...
export const STORAGE_KEYS = {
  ...
  // 🔴 2026-08-15 사장님: "선물상자는 난 한 번도 못 봤는데." 못 볼 만했다 — 선물상자라는 UI가
  // 아예 없었고(인사이트 배너 자체가 상자였다), 그 배너는 7초 뒤 자동으로 사라지는데 자동으로
  // 사라지면 당첨 판정을 아예 안 탄다. 즉 "7초 안에 눌러야 하고, 눌러도 30%"였다.
  // → 설계를 뒤집는다. **자주 뜨지만 대부분 꽝**에서 **드물지만 뜨면 무조건 진짜**로.
  //   보이는 상자를 만들어놓고 열 번 중 일곱 번이 비어 있으면 그게 더 나쁜 경험이다.
  //   오늘 선물이 있는 날인지를 하루에 한 번만 뽑아 여기 기록한다(`YYYY-MM-DD:yes|no`).
  //   화면을 다시 열 때마다 다시 뽑히면 결국 무한 재시도가 되므로 날짜와 함께 고정해야 한다.
  insightGiftOfferedDate: 'pace_insight_gift_offered_date',
