...
유발 — `feed/index.tsx` 상단 주석에 실측 근거 있음). 그래서 이번엔 "로딩을 없앤다"가 아니라
"로딩 중임을 명확히 보여준다"(스피너)까지만 하고 멈췄다 — 실제 지연을 없애려면 새로운 접근이
필요하고, 그건 이번 세션 범위 밖(사장님 판단 필요: 이 정도 지연을 감수할지, 아니면 다른 프리로드
전략을 다시 연구할지).
