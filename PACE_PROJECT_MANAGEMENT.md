...
`tsc --noEmit` 클린, Release 빌드 실기기 설치 완료. 실기기 진단 핑(`DIAG_BANNER_*`)을 심어뒀고
앱을 열면 로그로 실제 유닛 ID를 확인할 수 있다 — 아직 실기기 확인 전이라 사장님이 앱을 한 번
열어보면 좋겠다. **Android는 같은 파일을 안 건드렸으므로 별도로 실기기(또는 Play Console 리포트)
로 실 수익이 잡히기 시작하는지 확인 필요.**
