...
//
// ⚠️ iOS 핵심 제약과 대응(정상/비정상 처리):
//  - 폰을 엎어놓으면 iOS가 화면을 끄고 앱이 background로 → CoreMotion이 멈춘다. 그래서 background
//    동안엔 센서 이벤트가 없다. 대신 "엎어놓은 시각(flipStartMs)"을 기록해 두고, 앱이 foreground로
//    돌아와 실제로 집어든 걸(physicalFaceDown=false) 확인하는 순간 그 사이 경과 전체를 정산한다
//    (background 구간 브리징). 이 로직은 useFlipMode 훅이 AppState와 함께 처리한다.
