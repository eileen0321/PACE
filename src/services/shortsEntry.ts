...
async function resolveVideoIdWithPrefetch(
  ...
  //   서버가 시드를 못 주는 상황이 실제로 있었다(2026-08-28). 원인은 Vercel 빌드가 11일간
  //   실패해 낡은 코드가 서빙되던 것이고, 그동안 프록시가 0개를 줘 여기까지 내려와 null 이 됐다.
  //   ⚠️ 당시 나는 "유튜브가 데이터센터 IP 를 막는다"고 적었으나 **틀렸다** — 근거로 삼은
  //     비교가 한글 인코딩 오류로 오염된 측정이었다(api/youtube.ts 의 같은 주석 참고).
  //   폴백 자체는 유효하다. 서버가 어떤 이유로든 못 주면 앱이 멈추는 건 같기 때문이다.
