...
export interface OverlayService {
   ...
   * 깎는데 통계의 "오늘 사용 시간"은 세션 시작~현재의 순수 벽시계라 두 숫자가 어긋났다(세션만
   * 켜두고 안 보면 알약은 그대로인데 통계엔 시간이 쌓임). 사용자가 이해하는 사용 시간은 실제로
   * 본 시간이므로 통계를 알약 기준으로 통일한다 — 그 근거값을 네이티브가 누적해 여기로 준다.
   * iOS는 서드파티 앱의 재생 상태를 관찰할 수단이 OS 차원에 없어 null(호출부가 벽시계로 폴백).
   */
  getWatchedSeconds(): Promise<number | null>;
  /**
   * 2026-08-03 사장님 지시("유튜브 앱으로 열면 시간 측정이 안 되는 걸 알려줘야 하지 않냐") —
   * Pace 추적은 전부 세션에 묶여 있어, 사용자가 런처에서 유튜브를 직접 열면 아무것도 기록되지
   * 않고 그 사실조차 알 수 없었다. 이미 보유한 PACKAGE_USAGE_STATS로 오늘 지원 앱을 얼마나
   * 켜뒀는지 사후 조회해 분석 화면에서 Pace 기록 시간과 나란히 보여준다.
   * ⚠️ "켜둔 시간"(totalTimeInForeground)이지 "본 시간"이 아니다 — UI 문구를 그렇게 써야 한다.
   * iOS는 null: Screen Time 사용량은 DeviceActivityReport 샌드박스를 벗어날 수 없다는 게 애플의
   * 명시적 설계라 앱이 숫자를 읽어올 방법이 원천적으로 없다(웹 확인, 2026-08-03).
   */
  getSupportedAppForegroundSecondsToday(): Promise<number | null>;
  /**
