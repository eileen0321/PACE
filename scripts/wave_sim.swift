...
struct Sim {
  ...
  mutating func feedLuma(t: Double, l: Double, m: Double, r: Double) {
    ...
    // lumapass — 손-확인 게이트(모듈 미러): 최근 손 관측 있을 때만.
    if t - lastTriggerMs > passRefractoryMs, dipHistory.count >= 6, t - lastHandSeenMs <= 3000, let firstD = dipHistory.first, t - firstD.t >= 500 {
...
extension Sim {
  mutating func feedGrid(t: Double, g: [Double]) {
    ...
    let aspect = bw > 0 ? Double(bh) / Double(bw) : 0  // iOS 전치: 안드 bw/bh 대응 (모듈 미러)
    ...
    if fraction >= 0.05, fraction <= 0.30, cons >= 0.6, density >= densityTh, aspect <= 0.9, handSeen {
      ...
      if gridStandaloneEnabled { lastTriggerMs = t; events.append("FIRE gridpass") }
    }
  }
}
...
check("격자 세로 스윕 차단(안드 한방향 aspect)", s28b.events, fires: 0)
...
var s37 = Sim()
var t37 = 0.0
for _ in 0..<13 { s37.feedGrid(t: t37, g: flatGrid(150)); t37 += 80 } // 창 채움(조용)
var gb37 = flatGrid(150)
