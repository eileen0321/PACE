...
### 2026-09-03(Mac/iOS, 긴급-정정) — 🔴 격자 축 divergence 정정: 안드 현재값 verbatim 이식(하한/밀도/aspect)
- 직전 "손-부재 게이트" 접근은 **부족**했음(사장님 "지금도 계속 넘어가"). 실기기 재채증(diag 16:17~16:18):
  격자가 cells=2·4·5뿐 아니라 cells=23·29(폰이 머리와 함께 움직임)까지 발화. 손이 프레임에 있어(폰 쥠/턱 괴기)
  손-부재 게이트가 무력. → 손-부재 게이트/발화비활성 접근 폐기.
- **근본 원인 재확정**: iOS 격자 축이 08-28 스냅샷(하한 0.0078=2칸)에 멈춰 있었음. 안드(PaceHandWaveDetector.kt)는
  08-30/08-31에 같은 문제 겪고 이미 고쳐둠 — iOS에 이식이 안 됨. **3곳 갈라짐**:
  | 상수 | 안드 현재 | iOS(전) |
  |---|---|---|
  | GROSS_MOTION_CELL_FRACTION(하한) | **0.05(13칸)** | 0.0078(2칸) |
  | MIN_DENSITY_BIG | **0.15** | 0.30 |
  | MAX_ASPECT(가로세로비) | **≤0.9**(손=세로로 김) | 게이트 없음 |
- 조치: iOS checkGridMotion에 안드 세 값 verbatim 이식(gridFracMin 0.05, gridMinDensityBig 0.15,
  aspect=bw/bh ≤ gridMaxAspect 0.9 추가). 시뮬 34/34(격자 테스트 전면 개편: 세로손짓 발화 / 2·5칸·얼굴정사각·
  폰움직임정사각 전부 차단, 채증 회귀 2건 포함).
- ⚠️ **2번째 divergence 발견(미조치, 사장님 판단 요망)**: 안드 단독 발화 축은 **lumapass + gross-motion(격자) 둘뿐**.
  cross/sweep/occlusion은 안드가 08-30에 단독발화 **차단**("노이즈와 겹침"). 그런데 iOS는 cross/sweep/glide/nearpass/
...
### 2026-09-03(Mac/iOS) — 🟢 격자 aspect 게이트 방향 수정: iOS 전면카메라 버퍼 90° 전치 반영
- 축 정렬 후 사장님 "손짓 인식 못한다". 채증(diag 13:06:46~49 손짓): 진짜 손짓이 cons 0.88~1.00인데
  **aspect 2.4~4.3(가로로 긴 띠)**. 안드에서 이식한 게이트 aspect≤0.9(세로)라 전부 막힘.
- 원인: iOS 전면카메라 Y버퍼가 안드 대비 **90° 전치** — 같은 "훠이"가 안드=세로(0.45~0.80), iOS=가로(2.4~4.3).
- 조치: 안드 의도(얼굴=정사각 배제)는 유지하되 iOS 좌표계에 맞춰 **"정사각 근처(0.9~1.111)만 차단"**으로 변경
  (notSquare = aspect≤0.9 || aspect≥1.111). 가로·세로 어느 쪽 스윕이든 통과, 얼굴/정면접근(정사각)만 차단.
- 시뮬 35/35(가로 손짓·세로 손짓 발화 / 정사각 얼굴·폰움직임 차단 / 2·5칸·분산 차단).

### 2026-09-04(Mac/iOS, 긴급) — 🔴 차 안에서 손짓 없이 자동 전진 = 격자 지속모션 억제 추가
- 채증(diag 23:11~23:12 차 안): 손짓 없이 gridpass 20+회 연속(cells 13~59, cons 0.81~1.00). 차가 움직이며
  햇빛/그늘/풍경이 프레임을 균일하게 바꿔 격자가 손 스윕으로 오판. 안드도 gross-motion에 기기움직임 게이트
  없음(잠재 동일 버그) — iOS를 더 낫게 고침.
- PaceGesture엔 가속도계 없음 + speedSuppressUntilMs는 죽은 코드(할당된 적 없음). 그래서 **luma만으로 구분**:
  손짓=짧은 1회 전환, 차/이동=매 프레임 지속. checkGridMotion에 활동 추적(매 프레임 frac>=0.05 여부) +
  최근 2.5s 창에서 큰-변화 비율 >=45%면 지속모션으로 보고 발화 억제. 단일 스윕(≈0.4s)은 비율 낮아 통과.
- 시뮬 37/37(차-지속모션 억제 / 조용후 단일스윕 발화 회귀 추가).
- ⚠️ 빌드 18은 이 버그 포함인 채 이미 ASC 업로드됨 — 심사 제출 금지. 빌드 19로 교체 예정.

### 2026-09-06(Mac/iOS) — 🔴 안드 손-확인 게이트(fe2c279) iOS 이식 — 밝기 축은 손 봤을 때만 발화
- 안드가 8커밋 대개편(인식률 40→92%). iOS 네이티브/시뮬은 안 건드렸으나, **핵심 개념 하나가 iOS에도 필요**:
  안드 fe2c279 — lumapass·gross-motion(밝기 기반 축)은 판별기준이 "밝기 크기" 하나뿐이라 손 없이 발화
  (차·조명·얼굴). **LUMAPASS_HAND_RECENT_MS=3000: 최근 3초 내 실제 손 관측 있을 때만 발화**.
- iOS 이식: checkGridMotion + lumapass 발화에 `lastHandSeenMs<=3000` 게이트 추가(iOS는 Vision 손 관측을
  lastHandSeenMs로 이미 추적). 차/조명 환경(손 없음)에서 원천 차단. 기존 aspect·지속모션 억제와 병행(다층).
- 축 간 이중 넘김: iOS는 grid·lumapass가 lastTriggerMs(1.2s 불응) 공유라 구조적으로 이중 발화 방지 — 별도 불필요.
- 방향 게이트/traverse/렌즈가리기 세기 등은 안드 전용 축(MediaPipe) — iOS 무관.
- 시뮬 38/38(손미관측 차단 신규). tsc 0.

### 2026-09-06(Mac/iOS) — 🔴 격자 정면접근 차단(스윕 전용) — "앞에서만 움직여도 넘어감" 수정
- 채증(diag 04:48 wave_summary maxGrowth=2.44 = 손이 앞으로 다가옴): 격자 축(방향 무관)이 손의 정면 접근/
  등장 밝기 변화로 발화(gridpass cells 15~27). 사장님 "왼→오로 안 움직이고 앞에서만 움직였는데 넘어가".
- 격자는 "가로 스윕 vs 정면 접근"을 원리적으로 구분 못 함(둘 다 밝기 변화). iOS 감지를 **방향성 있는
  lumapass(L→M→R 순차 = 가로 스윕만, 정면 동시변화는 순서조건서 탈락)로 일원화**. 격자 단독발화 off
  (gridStandaloneEnabled=false, 계산·진단 유지). 새 가이드("왼→오 스치듯")와 정확히 일치.
- 안드는 격자 켜두지만(방향게이트 포기) iOS는 사장님 사양(스윕만 넘김) 우선 — 격자가 정면접근을 못 걸러 상충.
- 손짓은 이미 "실험(Beta)" 표시 + 포커스온=자동넘기기 기본이라, 오발화 최소화가 우선.
- 시뮬 38/38(격자 단독발화 off 반영, lumapass가 스윕 발화 검증). tsc 0.

### 2026-09-06(2차) — 🔴 격자 재활성 + 가로이동 게이트 — "왜 안돼"(격자 끄니 진짜 스윕도 죽음) 수정
- 앞서 격자를 통째로 끄니(스윕 전용 lumapass 의존) 사장님 진짜 스윕도 안 잡힘(채증 diag 05:59 maxSweep=1.35 =
  진짜 가로 스윕인데 무발화). lumapass는 L→M→R 순차 조건이 까다로워 실스윕을 자주 놓침.
- 해결: 격자 재활성(gridStandaloneEnabled=true) + **가로이동 게이트** — 랜드마크 sweep(x범위/손크기)이 최근
  900ms 내 0.6 이상일 때만 격자 발화. 정면 접근(가로 이동 없음)은 차단, 가로 스윕(sweep 큼)은 통과.
  단일 스와이프(반전 0회)도 잡으려 반전 요구하는 sweep축이 아니라 raw sweep 값 사용.
- 다층 방어 최종: 손-확인(3s) + aspect(정사각 배제) + 지속모션 억제(차) + 가로이동(정면접근 배제).
- 시뮬 39/39(실스윕 발화 / 정면접근 차단 신규). tsc 0.

### 2026-09-06(3차) — 🔴 안드 verbatim 복귀 (사장님 "안드처럼 하라니까") — 방향 비대칭·무발화 수정
- 증상: 오→왼만 되고 왼→오 안 됨(방향 비대칭) + 마지막 빌드는 아예 무발화.
- 원인: 내가 안드에 없는 것들을 덧붙임 — 지속모션 억제, **가로이동 게이트(랜드마크 sweep이 방향별 추적
  품질에 의존 → 한 방향만 통과 = 비대칭 주범)**, 양방향 notSquare. 안드는 방향 게이트를 포기(방향 무관=대칭).
- 조치: 격자 검출을 안드 checkGrossMotion **verbatim**으로 복귀 — 하한 0.05·상한 0.30·밀도 0.9/0.15·
  aspect≤0.9·손확인 게이트(fe2c279). 내 발명품 3종 전부 제거. iOS 하드웨어 실측 차이 2곳만 보정:
  ① aspect=bh/bw(iOS 전면버퍼 90° 전치 — 안드 bw/bh≤0.9 대응), ② 일관성 0.8→0.6(iOS는 화면빛 손 밝기
  섞임, 실측 0.79 — diag 06:07). 차/조명 방어는 안드처럼 손확인 게이트가 담당.
- 시뮬 37/37(안드 verbatim 반영: 세로 스윕은 한 방향 aspect로 차단, 정사각/손미관측 차단, 가로 스윕 발화).
- ⚠️ 정면 접근(앞에서만)은 안드처럼 aspect로만 거름 — 안드와 동일 거동. 재현되면 안드 세션과 함께 재검토.

### 2026-09-06(4차) — 🔴 격자 returndrop 이식 — "안드는 오왼 반응 안 하는데 넌 왜 넘어가냐"
- 격자는 방향 무관이라 왕복(왼→오·오→왼) 둘 다 발화 → 리턴 스트로크도 넘어감. 안드는 오왼(리턴) 무시.
- 조치: 안드 cross의 returndrop 이식. 손 x이동 방향(랜드마크 netX/손크기, ±0.4)을 추적해 **직전 발화의
  반대 방향 스트로크를 1회 무시**(상대 방향이라 절대 부호 추측 불필요, 1회 소비 후 리셋=자가복구, 창 2.5s).
  왼→오 발화 → 되돌아오는 오→왼 억제 = 한 동작 한 발화. 같은 방향 반복(진짜 다음 손짓)은 그대로 발화.
...
### 2026-09-06(7차) — 🟢 채증 세션 결과로 sweep 축 확정 (라벨 데이터 기반)
- 채증(captureMode: 발화차단+capH/capG/capFIRE 로깅). 사장님 6단계 라벨 동작. 핵심 발견:
  | 동작 | 손크기 | sweep(가로폭) | netdx(순이동) | 손존재 |
  |---|---|---|---|---|
  | 1-3 가만히/몸/얼굴 | — | — | — | **손 없음(capH 0)** |
  | 4 손들고 가만히 | 0.16 | 0.27~0.47 | 0.04 | O |
  | 5-6 스윕 | 0.15~0.25 | **1.37~1.62** | 0.17~0.23 | O |
- 결정적: ① 사장님 손짓은 **왕복(훠이)** — sweep(가로폭) 큼(1.3~1.6)인데 netdx(순이동) 작음(0.2) → 방향(netdx)
  기반은 원리적 불가. ② 손짓 vs 손들고가만히 = **sweep**(1.3~1.6 vs 0.3~0.5). ③ 몸/얼굴은 손 자체가 없음.
  ④ NEAR 게이트(0.20)가 사장님 손(0.15~0.17)보다 높아 luma 축을 다 막고 있었음(최근 "안 됨" 원인).
- 조치: **sweep 축 ON**(landmark 왕복폭, 손 필요·반전 필요·불응). sweepRatioThreshold 0.16→**0.8**(손짓 1.3~1.6 /
  가만히 0.3~0.5 사이). 격자·lumapass·cross·glide·nearpass·occlusion 전부 OFF. captureMode OFF.
  → 몸/얼굴(손없음)=무발화, 가만히(낮은sweep)=무발화, 왕복 한 번=한 발화(반전+불응 1.2s).
- ⚠️ 시뮬은 sweep 축 미모델(cross/lumapass/grid만) — 이 변경은 실기기 검증이 핵심. 안드는 sweep 단독발화 막았으나
  (부호 불신) iOS 사장님 손짓이 왕복이라 sweep(방향무관 왕복폭)이 오히려 정답.

### 2026-09-06(8차) — 🟢 cross 방향 게이트 확정 — "오왼도 넘어감" 수정 (채증 부호)
- 채증 방향 확정(diag): 왼→오(5단계 13:07:33) = x 증가·**netdx 양수(+0.10~+0.23)**, 오→왼(6단계 13:08:01) =
  x 감소·**netdx 음수(-0.10~-0.17)**. → cross를 **segNet 양수(왼→오)일 때만 발화**(crossWaveDir=1).
- 손짓은 단방향 스와이프(straight=1.0)라 cross 축(netdx 기반) 사용, sweep 축(반전 요구)은 부적합.
- ⚠️ 자세 바뀌면 부호 뒤집힐 수 있음(안드 경고) — 그땐 crossWaveDir만 -1로.

### 2026-09-06(9차) — 🔴 방향 게이트 포기 확정(안드와 동일 결론) — 부호 뒤집힘 실증
- crossWaveDir=1(왼→오=양수만) 넣었더니 "3번째부터 안 됨". 로그(diag 13:22~23): 같은 왼→오가 net +0.29→
  +0.31(발화)이다가 갑자기 전부 net 음수(-0.10~-0.23, crossdir skip). **동일 손짓인데 segNet 부호가 뒤집힘.**
- = 안드가 실측으로 확인하고 포기한 현상("부호가 자세에 따라 뒤집힌다"). 카메라로 왼↔오 방향 신뢰성 구분 불가.
- 조치: crossWaveDir=0(양방향) + returndrop(리턴 억제) = 한 동작 한 발화. "오왼만 차단"은 이 방식으로 불가 —
  결정적 제어는 블루투스 리모컨. 손짓은 "실험" 표시 유지.

### 2026-09-06(10차) — 🔴 카메라 손짓 발화 OFF (한계 도달, 정직한 중단)
- 하루 ~20빌드. 모든 축이 iOS서 실패(사장님 데이터로 실증): 격자/lumapass=몸/얼굴/조명 오발화, cross=부호
  뒤집힘(방향고정 불가)+returndrop 켜면 정방향 죽고 끄면 막 넘어감, sweep=단방향(반전0) 미발화.
- 원격 로그튜닝의 한계(카메라/손짓 재현 불가). 카메라 손짓 발화 전부 OFF(crossStandalone=false, 나머지도 off).
  넘김=BT 리모컨+자동넘기기(결정적). 손짓은 "실험" 표시 유지. 볼륨키/자동넘김은 별도 모듈이라 정상.
- 재개하려면: 온디바이스 CV 개발 사이클(라벨 데이터셋+반복 실측) 필요. captureMode 인프라는 남겨둠.
