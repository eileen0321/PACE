...
### 다른 앱들은 어떻게 하나 — 웹 조사 (사용자 지시, 2026-07-20)

ScreenZen/Opal 등 실제 스크린타임/디지털 웰빙 앱들을 조사한 결과:
- **오버레이 + 접근성 서비스 조합 자체는 이 카테고리의 표준 패턴이다.** ScreenZen 공식 설명에도
  "디스플레이 오버레이 접근 권한 + 접근성 서비스 권한이 필요하며, 이를 이용해 앱/웹사이트를
  시스템 레벨에서 감지하고 차단한다"고 명시돼 있다 — Pace의 오버레이+`PaceAccessibilityService`
  구조가 업계에서 딱히 특이하거나 위험 신호로 보이는 조합이 아니라는 근거.
- **그러나 "탭 한 번으로 다음 영상 넘기기" 같은 원격 조작 버튼을 제공하는 사례는 검색으로 못
  찾았다.** ScreenZen/Opal류는 전부 "지연시키기·차단하기·리다이렉트하기" 방향으로만 설계돼
  있다 — 콘텐츠 소비를 더 매끄럽게 만들어주는 기능은 이 카테고리의 제품 철학과 반대 방향이다.
- **시사점**: 알약의 Next/Prev 버튼은 스와이프보다 편한가라는 질문(사용자가 이미 제기)에 더해,
  "이게 애초에 이 제품군이 하는 일이 맞는가"라는 더 근본적인 질문이 있다. Pace의 핵심 가치는
  "덜 보게 돕기"인데, Shorts 넘기기를 더 쉽게 만드는 버튼은 그 반대 방향으로 작용할 여지가
  있다. AUTO ON/OFF 토글(손 안 대고 계속 보게 하거나 반대로 완전히 멈추게 하는 기능)은 같은
  선상이지만 "관리 도구"로 포지셔닝하기 쉬운 반면, Next/Prev는 순수 "편의 기능"이라 이 구분이
...
### 실기기 검증 10차 — Focus Session 전체 스택 실사용 검증 + 알약 배지 동기화 버그 (2026-07-20)

10분 제한 로직을 실제로 검증하기 위해 타이머를 8초로 임시로 줄여서 실기기로 테스트했다(10분을
실제로 기다릴 수 없어서). 결과: **자동 시작/자동 종료/수동 취소 전부 정상 작동** — 단, 이 과정에서
진짜 버그 하나를 잡았다: 10분 타이머가 자동으로 꺼질 때 토스트("Focus Session Ended")는 정상
떴지만, 알약의 "AUTO ON" 배지 텍스트는 안 바뀌고 그대로 남아있었다. 원인: 배지 탭 핸들러는 자기가
직접 `autoNextEnabled` 필드를 바꾸고 `applyAutoBadgeStyle()`도 불렀지만, `setAutoMode()`를 "밖에서"
부르는 경로(10분 타이머 등)는 이 동기화를 안 태웠다. **고침**: 동기화 로직을 `setAutoMode()`
자체로 옮겨서 모든 호출 경로(탭/Bluetooth/타이머)가 배지 상태를 일관되게 반영하게 통일. 8초
재테스트로 재확인 완료(토스트+배지 동시 정확히 전환), 10분으로 원복.

이어서 **자동재생(위치 기반 Tier1 워처)과 핑거스냅 둘 다 실기기 로그로 실제 작동 확인**:
- `SWIPE tier=1 reason=near-end current=27s total=28s` — 영상이 끝나기 직전 실제로 자동 스와이프됨,
  직후 새 영상(다른 total 값)으로 타이밍 추적 재시작까지 확인.
- `PaceSnapDetector`가 조용한 구간에서 실제 손가락 스냅을 여러 차례 정확히 감지(`passed=true`,
  주파수 대역비 판정 통과) → `triggerNext()` → 실제 스와이프까지 이어지는 전체 파이프라인 확인.

**핑거스냅의 진짜 한계(V1, 미해결)**: 영상 소리가 켜져 있으면 그 소리 자체가 마이크(VOICE_RECOGNITION
소스)에 계속 섞여 들어와 배경 노이즈 바닥이 계속 오르고(200→500 이상), 스냅 특유의 스파이크가
묻히거나 주파수 판정에서 걸러진다(`passed=false`). 조용한 환경에서는 잘 되지만, 실사용(소리 켜고
시청)에서는 안정성이 배경음 종류에 따라 들쭉날쭉하다 — 단순 에너지+주파수 휴리스틱의 근본적
한계이고, 제대로 하려면 스피커 출력을 마이크 입력에서 빼는 에코캔슬링 수준 DSP나 ML 기반 소리
분류가 필요하다(오늘 밤 범위 밖). **알려진 V1 제약으로 남겨둠.**

**알약 "됐다 안됐다" 현상 — 두 가지 원인 확인 후 각각 처리**:
1. **표시 로직이 UsageStatsManager 1초 폴링에만 의존**(공식 문서 기준으로도 실시간 정확도 미보장)
   — `PaceAccessibilityService`가 이미 `TYPE_WINDOW_STATE_CHANGED` 이벤트로 foreground 패키지를
   즉시 추적하고 있었으므로(`onAccessibilityEvent`), 이 값을 우선 쓰도록 `getCurrentForegroundPackage()`
   companion 함수로 노출하고 `PaceOverlayService`의 폴링이 이걸 먼저 쓰게(없으면 UsageStatsManager로
   폴백) 고쳤다 — 이벤트 기반이라 폴링 지연 없이 즉시 반영됨.
2. **프로세스 자체가 재시작되면(멀티윈도우 전환, 메모리 정리 등) `overlayView`가 null이 되고 최대
   60초(다음 정기 틱까지) 알약이 안 보이는 구간이 있다** — 이미 있던 `restoreIfNeeded()`/
   `ensureInfraReady()` 복구 로직 자체는 정상 작동 확인(로그로 재생성 확인), 다만 이 60초 지연은
   구조적 한계라 이번엔 손 안 댐(자연 복구되므로 영구 장애 아님).
※ 참고: 이 검증 도중 뜬 `AccessibilityUserState: Crashed service` 로그는 실제 코드 크래시가 아니라
`adb install -r`이 프로세스를 강제 종료한 걸 시스템이 그렇게 로깅한 것— 오인 주의.

### 손짓(조도 센서) Hands-Free — 시도 후 폐기 (2026-07-20)

사용자 제안(다른 AI 초안 기반): 새 무거운 의존성(MediaPipe 등) 없이 조도 센서(`Sensor.TYPE_LIGHT`)
로 카메라 앞 손짓을 감지하자는 아이디어. 검증 결과 조도 센서 자체는 이 기기에 실존(`TMD4907`)하고
`expo-sensors`의 `LightSensor`도 실재하지만(단, iOS는 완전 스텁이라 안드로이드 전용 — "양쪽 다
된다"는 원안 주장은 틀림), **JS 레이어로는 실제 유튜브 앱이 전면일 때(Pace 백그라운드) 동작을
멈추므로**(핑거스냅과 동일한 이유) 네이티브(Kotlin `SensorManager`)로 직접 구현했다
(`PaceLightWaveDetector.kt`).

**V1(절대 임계값, 손이 센서를 거의 완전히 가려야 인식)** → 사용자 피드백으로 "30cm 떨어진 거리에서
되냐"는 핵심 질문 발생 — 실사용 시나리오(거치대에 놓고 손을 안 대고 조작)에 이 기술 자체가 안 맞을
수 있다는 지적. **V2(상대 임계값, baseline 대비 40% 급락 — 핑거스냅의 noiseFloor 배수 판정과
동일 원리로 개선)**로 재구현했으나, 실기기 재검증 결과 **30cm 거리에서는 조도 값이 전혀 안
움직임**(약 40lux 저녁 실내 조명 조건에서 43~44 그대로 유지, 드롭 없음) — 이 밝기 수준에서는
손이 센서에 거의 닿을 정도로 가까워야만 감지되는 것으로 최종 확인됨.

**최종 결론(사용자 판단, "그럼 스와이프랑 머가 달라")**: 손을 폰 가까이 대야 한다면 그냥
스와이프하는 것과 실질적 차이가 없다 — 애초에 이 기능을 찾던 이유(거치대에 놓고 손 안 대고
조작)를 충족 못 하므로 **가치가 없다고 판단해 폐기**. `PaceLightWaveDetector.kt` 삭제, 관련 배선
(`PaceOverlayService.setAutoMode`/`onDestroy`, `PaceAccessibilityService.isPastHalfway`) 전부
제거하고 커밋 안 함 — 시도했다는 사실과 결론만 기록.
...
확인(WebView 코드 작성 전 1차 스크리닝):
- **TikTok** (`tiktok.com/foryou`): **로그인 없이 완전히 재생됨** — 유튜브 원본페이지와 정확히 같은
  패턴. 즉 기술적으로는 YouTube와 동일하게 "직접 네비게이션하면 됨" → 정책 위험도 동일하게 적용
  (남의 사이트 감싸기 + 컨트롤 오버레이). TikTok 개발자 가이드라인은 위반 시 "즉시 영구 밴"을
  명시하고 있어(웹 검색 확인) 오히려 유튜브보다 집행이 더 엄격할 수 있음.
- **Instagram** (`instagram.com/reels/`): **비로그인 상태로는 아예 로그인 페이지로 강제 리다이렉트**
  — 유튜브/틱톡처럼 "일단 콘텐츠는 보임" 단계에도 못 감. Pace WebView에서 되게 하려면 사용자의
  실제 인스타 계정으로 로그인시켜야 하는데, 이건 정책 위반 이전에 **제3자 앱에 자격증명을 맡기는
  신뢰/보안 문제**라 별도로 훨씬 더 위험함.

**결론**: 세 플랫폼(YouTube/TikTok/Instagram) 전부 WebView 직접 네비게이션 경로가 실질적으로
막혀 있거나(Instagram) 기술적으론 되지만 정책 위반(YouTube/TikTok) — 이 라인업 전체가 Plan B
...
### 런치 플로우 단순화 — 콜드 스타트가 탭 대신 바로 세션 화면으로 (2026-07-21 밤, 사용자 지시)
...
1. 앱 실행                        1. 앱 실행
   ↓                                 ↓
2. 바로 Overlay                   2. 바로 WebView(Pace Feed)
   ↓                                 ↓
3. YouTube 실행                   3. 버즈 탭 또는 리모컨
   ↓                                 ↓
4. 리모컨 또는 버즈 탭            4. 다음 영상
   ↓
5. 다음 영상
(핑거 스냅은 일단 보류 — 삭제하지 말고 살려서 나중에 테스트)
```

**분기 규칙(둘 다 적용)**: 맨 처음(온보딩을 아직 안 겪은) 실행에서만 온보딩 → 앱(Home 탭)으로
들어간다. **두 번째 실행부터** 콜드 스타트가 곧바로 세션 화면(Android=Overlay+YouTube 자동실행,
iOS=Pace Feed WebView)으로 간다 — 매번 온보딩 화면 반복이 아니라 "한 번 겪고 나면 그 다음부턴
바로 본론"이라는 뜻.

**로그인/유료화 체크**: 사용자 확인 결과 "완전히 스킵" — 그런데 실제로는 원래도 `index.tsx`/
`_layout.tsx` 어디에도 로그인을 강제하는 리다이렉트가 없었다(게스트 폴백이 `useUserStore.init()`
에서 이미 자동 처리되고 있었음, 그렙으로 확인). 즉 추가 코드 변경 없이 이미 요구사항 충족 —
paywall은 원래도 Settings 등에서 사용자가 직접 진입해야만 뜨는 모달이지 블로킹 게이트가 아니다.

**기존 탭 네비게이션은 완전히 유지** — 다만 콜드 스타트의 기본 목적지가 아니게 됐을 뿐, Home/
Focus/Stats/Settings 전부 그대로 존재한다. 진입 경로: `overlay/index.tsx` 오른쪽 위에 새로
추가한 원형 "P" 앱 아이콘 버튼 → 탭으로 다시 들어감(세션 자체는 끊기지 않고 백그라운드에서 계속
진행 — 네이티브 오버레이/YouTube가 자기 완결적이므로).

**핑거 스냅**: 사용자 지시대로 코드/배선 전혀 건드리지 않음(`PaceSnapDetector.kt`,
`PaceOverlayService.setAutoMode`의 `PaceSnapDetector.start()` 호출 그대로 유지) — 애초에
블루투스 리모컨/미디어버튼(`PaceOverlayService`의 MediaSession 콜백)이 이미 스냅과 완전히
독립적인 별도 트리거 경로였으므로, "블루투스를 주력으로, 스냅은 보류"라는 요구사항은 코드
변경 없이 이미 충족돼 있었다 — 스냅을 끄거나 숨기지 않았고, 단지 새 런치 플로우가 블루투스
트리거에 의존하지 않을 뿐(둘 다 세션이 켜져 있는 동안 항상 같이 활성).

**구현 범위(이 세션=Android만)**: `src/app/index.tsx`(콜드스타트 목적지 분기),
`src/app/onboarding/index.tsx`(`finish()` 목적지는 그대로 Home 유지 — 온보딩 직후는 앱으로),
`src/app/overlay/index.tsx`(앱 아이콘 버튼 추가). **iOS(`/feed` WebView 콜드스타트 분기)는 맥
세션이 동시에 구현 중**이라 `index.tsx`의 iOS 분기는 건드리지 않고 기존 `/(tabs)/home`
그대로 둠 — 맥 쪽 작업이 커밋되면 이 파일의 `Platform.OS === 'android'` 분기 옆에 iOS 쪽도
맞춰 넣을 것(같은 파일이라 병합 시 충돌 가능성 있음, 이 문서가 그 조율 근거).

`TEST_ALWAYS_SHOW_ONBOARDING`(index.tsx)을 `true`(매번 온보딩 강제, 아이콘/스플래시 시각 검토용)
→ `false`로 되돌렸다 — 켜진 채로는 "온보딩 최초 1회, 그 다음부턴 바로 세션"이라는 이번 분기
자체를 테스트할 방법이 없었기 때문(항상 온보딩 브랜치만 타서 "다음 실행" 분기에 영영 도달 못 함).
